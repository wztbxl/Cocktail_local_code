...
void plotCocktail_pT(TString cenName = "080", Bool_t withRho = kFALSE){
	...
	const Double_t dM = 0.01;//Scale by pT
	...
	const Double_t Nbinary[nCenBins] = {256.52, 811.80, 342.06, 51.22, 82.43, 19.49, 24.12, 11.83}; // Au+Au 54 GeV collisions
	...
	double dyCorss = 26.19*(9.88/13.44)*1.e-6;
	const Double_t dNtotdy = (dyCorss/35)*Nbinary[mCenIdx]; // 42nb/42mb
	...
	TString Filename = Form("./dndy%d%d.dat", CentralityLow[mCenIdx], CentralityHi[mCenIdx]);
	ifstream indata(Filename.Data()); //this dndy is AuAu@200GeV minibias(0-80%) results
	...
	if(indata.is_open()){
		...
		cout<<Form("AuAu@200 GeV %d-%d%% particle yields:", CentralityLow[mCenIdx], CentralityHi[mCenIdx])<<endl;
	...
	TPDF *ps; 
	TString dir;
	system(Form("mkdir -p output/Cen%d%d_pT", CentralityLow[mCenIdx], CentralityHi[mCenIdx]));
	dir = Form("output/Cen%d%d_pT", CentralityLow[mCenIdx], CentralityHi[mCenIdx]);
	if(withRho){
		ps = new TPDF(Form("%s/cen%d%d_pT_cocktail_withRho.pdf", dir.Data(), CentralityLow[mCenIdx], CentralityHi[mCenIdx]), 111);
	}else{
		ps = new TPDF(Form("%s/cen%d%d_pT_cocktail_withoutRho.pdf", dir.Data(), CentralityLow[mCenIdx], CentralityHi[mCenIdx]), 111);
	}
	ps->Off();

	//dalitz decay
	TFile *fpi0dal = new TFile(Form("/Users/wztbxl/Documents/work/run18/54GeV/plots/Cocktail/input/54Result/Cen%d%d/pi0dalitzAll.root", CentralityLow[mCenIdx], CentralityHi[mCenIdx]));
	if(!(fpi0dal->IsOpen())){
		cout<<"Failed to open the pi0 dalitz decay file !"<<endl;
		return;
	}
	TFile *fetadal = new TFile(Form("/Users/wztbxl/Documents/work/run18/54GeV/plots/Cocktail/input/54Result/Cen%d%d/etadalitzAll.root", CentralityLow[mCenIdx], CentralityHi[mCenIdx]));
	if(!(fetadal->IsOpen())){
		cout<<"Failed to open the eta dalitz decay file !"<<endl;
		return;
	}
	TFile *fomegadal = new TFile(Form("/Users/wztbxl/Documents/work/run18/54GeV/plots/Cocktail/input/54Result/Cen%d%d/omegadalitzAll.root", CentralityLow[mCenIdx], CentralityHi[mCenIdx]));
	if(!(fomegadal->IsOpen())){
		cout<<"Failed to open the omega dalitz decay file !"<<endl;
		return;
	}
	TFile *fetaprimdal = new TFile(Form("/Users/wztbxl/Documents/work/run18/54GeV/plots/Cocktail/input/54Result/Cen%d%d/etaprimdalitzAll.root", CentralityLow[mCenIdx], CentralityHi[mCenIdx]));
	if(!(fetaprimdal->IsOpen())){
		cout<<"Failed to open the etaprim dalitz decay file !"<<endl;
		return;
	}
	TFile *fphidal = new TFile(Form("/Users/wztbxl/Documents/work/run18/54GeV/plots/Cocktail/input/54Result/Cen%d%d/phidalitzAll.root", CentralityLow[mCenIdx], CentralityHi[mCenIdx]));
	if(!(fphidal->IsOpen())){
	...
	TFile *fomega2ee = new TFile(Form("/Users/wztbxl/Documents/work/run18/54GeV/plots/Cocktail/input/54Result/Cen%d%d/omega2eeAll.root", CentralityLow[mCenIdx], CentralityHi[mCenIdx]));
	if(!(fomega2ee->IsOpen())){
		cout<<"Failed to open the omega two body decay file !"<<endl;
		return;
	}
	TFile *fphi2ee = new TFile(Form("/Users/wztbxl/Documents/work/run18/54GeV/plots/Cocktail/input/54Result/Cen%d%d/phi2eeAll.root", CentralityLow[mCenIdx], CentralityHi[mCenIdx]));
	if(!(fphi2ee->IsOpen())){
		cout<<"Failed to open the phi two body decay file !"<<endl;
		return;
	}
	TFile *fjpsi2ee = new TFile(Form("/Users/wztbxl/Documents/work/run18/54GeV/plots/Cocktail/input/54Result/Cen%d%d/jpsi2eeAll.root", CentralityLow[mCenIdx], CentralityHi[mCenIdx]));
	if(!(fjpsi2ee->IsOpen())){
		cout<<"Failed to open the jpsi two body decay file !"<<endl;
		return;
	}
	/*TFile *fpsi2ee = new TFile(Form("/Users/wztbxl/Documents/work/run18/54GeV/plots/Cocktail/input/Cen%d%d/psi/psi2eeAll.root", CentralityLow[mCenIdx], CentralityHi[mCenIdx]));
	if(!(fpsi2ee->IsOpen())){
	...
	TFile *fcc = new TFile(Form("/Users/wztbxl/Documents/work/run18/54GeV/plots/Cocktail/input/54Result/Cen%d%d/cc2eeAll.root", CentralityLow[mCenIdx], CentralityHi[mCenIdx]));
	if(!(fcc->IsOpen())){
		cout<<"Failed to open the ccbar decay file !"<<endl;
		return;
	}
	TFile *fdy = new TFile(Form("/Users/wztbxl/Documents/work/run18/54GeV/plots/Cocktail/input/54Result/Cen%d%d/dy2eeAll.root", CentralityLow[mCenIdx], CentralityHi[mCenIdx]));
	if(!(fdy->IsOpen())){
	...
	*/
	...
	if(withRho) {
		...
		f= new TFile(Form("%s/cen%d%d_cocktail_withoutRho_pT.root", dir.Data(), CentralityLow[mCenIdx], CentralityHi[mCenIdx]),"recreate");
	}
	f->cd();
	hMCAcc0MvsPt->Write();
	hMCAcc1MvsPt->Write();
